...
¿Por qué querríamos cubrir más reglas en una misma frase? Más adelante veremos que una frase grabada, que cubre una regla, aportará información solamente de esa regla en particular. Si una frase cubre varias reglas a la vez estaríamos obteniendo más información ya que ésta aportaría para cada una de las reglas. En ambas opciones utilizamos sólo una grabación. Por eso es importante que en cada elección de cada frase a grabar se maximice su cubrimiento. 
...
Mientras haya frases sin ser seleccionadas para grabar, elegimos una frase de la regla que menos está cubierta. O sea, elegiremos como próxima regla a cubrir la que tenga menor porcentaje de cubrimiento. Esto se realiza llamando al método \textit{ObtenerReglaConMenorPorcentaje} del objeto \textit{DicPct}. Si hay varias con el mismo mínimo porcentaje, se elige una al azar.
...
\begin{figure}[H]
	...
	width=14cm,
	height=10cm,
	title={Cubrimiento de reglas a través del experimento},
	xlabel={Cantidad de grabaciones},
	ylabel={Porcentaje de muestras grabadas para cada regla},
	xmin=0, xmax=15,
	ymin=0, ymax=100,
