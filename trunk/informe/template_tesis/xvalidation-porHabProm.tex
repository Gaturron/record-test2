...
Agregamos un hablante más de Buenos Aires ya que de esta manera cuando quitamos un hablante para testeo el algoritmo ZeroRule posee instancias equilibradas en el conjunto de entrenamiento. De esta forma es más correcto el análisis. De esta forma, el conjunto de datos posee sólo una unidad de diferencia entre ambos grupos. La elección de cada uno de estos hablantes fue al azar. Como el esquema anterior, tendremos un fold por cada hablante. 
...
Los resultados de este cross-validaton podemos observarlos en la tabla \ref{HPTDT_clas_xval_porHab} para la variante utilizando hablantes equilibrados, y en la tabla \ref{HPTDT_clas_xval_porHab} para todos los hablantes.
