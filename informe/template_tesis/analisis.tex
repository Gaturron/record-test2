...
\begin{figure}[H]
	...
	\node (prom) [prom, below of=test3] {Promedio};
	
	\draw [arrow] (test1) |- (prom);
	\draw [arrow] (test2) -- (prom);
	\draw [arrow] (test3) -- (prom);
	\draw [arrow] (test4) -- (prom);
	\draw [arrow] (test5) |- (prom);
...
Para los vectores provenientes de una distribución Normal se le aplica este test. Este nos provee una forma de determinar si dos conjuntos de test son significativamente distintos. De la misma forma que planteamos la hipótesis del test de Wilcoxon, este va a tener las mismas hipótesis. O sea: 
